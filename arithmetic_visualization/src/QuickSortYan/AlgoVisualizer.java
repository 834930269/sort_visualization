...
public class AlgoVisualizer {
    ...
    private int partition(int l,int r){
            int v=data.get(l);
            setData(l,r,-1,l,r,-1);
            int temp=data.get(l),low=l,high=r;

            while(low<high){
                while(low<high && data.get(high)>=temp){
                    high--;
                    setData(l,r,-1,low,high,temp);
                }
                data.set(low,data.get(high));
                while(low<high && data.get(low)<=temp){
                    low++;
                    setData(l,r,-1,low,high,temp);
                }
    ...
    private void setData(int l,int r,int fixedPivot,int low,int high,int temp){
        data.l=l;
        data.r=r;
        if(fixedPivot!=-1)
